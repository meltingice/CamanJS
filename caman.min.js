...
r=parseInt(hex.substr(0,2),16);g=parseInt(hex.substr(2,2),16);b=parseInt(hex.substr(4,2),16);return{r:r,g:g,b:b};},processKernel:function(adjust,kernel,divisor,bias){var val={r:0,g:0,b:0};for(var i=0;i<adjust.length;i++){for(var j=0;j<adjust[i].length;j++){val.r+=(adjust[i][j]*kernel[i][j].r);val.g+=(adjust[i][j]*kernel[i][j].g);val.b+=(adjust[i][j]*kernel[i][j].b);}}
...
Caman.events.fn[_type].push(_fn);return true;}},cache:{}};(function(Caman){Caman.forEach(["trigger","listen"],function(key){Caman[key]=Caman.events.fn[key];});})(Caman);var ProcessType={SINGLE:1,KERNEL:2};Caman.manip.pixelInfo=function(loc,self){this.loc=loc;this.manip=self;};Caman.manip.pixelInfo.prototype.getPixelRelative=function(horiz_offset,vert_offset){var newLoc=this.loc+(this.manip.dimensions.width*4*(vert_offset*-1))+(4*horiz_offset);if(newLoc>this.manip.pixel_data.length||newLoc<0){return{r:0,g:0,b:0,a:0};}
...
return rgba;});};Caman.manip.greyscale=function(){return this.process({},function greyscale(adjust,rgba){var avg=0.3*rgba.r+0.59*rgba.g+0.11*rgba.b;rgba.r=avg;rgba.g=avg;rgba.b=avg;return rgba;});};Caman.manip.contrast=function(adjust){adjust=Math.pow((100+adjust)/100,2);return this.process(adjust,function contrast(adjust,rgba){rgba.r/=255;rgba.r-=0.5;rgba.r*=adjust;rgba.r+=0.5;rgba.r*=255;rgba.g/=255;rgba.g-=0.5;rgba.g*=adjust;rgba.g+=0.5;rgba.g*=255;rgba.b/=255;rgba.b-=0.5;rgba.b*=adjust;rgba.b+=0.5;rgba.b*=255;if(rgba.r>255){rgba.r=255;}else if(rgba.r<0){rgba.r=0;}
...
return rgba;});};Caman.manip.curves=function(chan,start,ctrl1,ctrl2,end){var Ax,Bx,Cx,Ay,By,Cy,x0=start[0],y0=start[1],x1=ctrl1[0],y1=ctrl1[1],x2=ctrl2[0],y2=ctrl2[1],x3=end[0],y3=end[1],t,curveX,curveY;Cx=3*(x1-x0);Bx=3*(x2-x1)-Cx;Ax=x3-x0-Cx-Bx;Cy=3*(y1-y0);By=3*(y2-y1)-Cy;Ay=y3-y0-Cy-By;if(typeof chan==='string'){if(chan=='rgb'){chan=['r','g','b'];}else{chan=[chan];}}
bezier={};for(var i=0;i<1000;i++){t=i/1000;curveX=Math.round((Ax*Math.pow(t,3))+(Bx*Math.pow(t,2))+(Cx*t)+x0);curveY=Math.round((Ay*Math.pow(t,3))+(By*Math.pow(t,2))+(Cy*t)+y0);if(curveY>255){curveY=255;}else if(curveY<0){curveY=0;}
bezier[curveX]=curveY;}
if(start[0]>0){for(i=0;i<start[0];i++){bezier[i]=start[1];}}
if(end[0]<255){for(i=end[0];i<=255;i++){bezier[i]=end[1];}}
var leftCoord,rightCoord,j,slope,bint;if(bezier.length<256){for(i=0;i<=255;i++){if(typeof bezier[i]==="undefined"){leftCoord=[i-1,bezier[i-1]];for(j=i;j<=255;j++){if(typeof bezier[j]!=="undefined"){rightCoord=[j,bezier[j]];break;}}
bezier[i]=leftCoord[1]+((rightCoord[1]-leftCoord[1])/(rightCoord[0]-leftCoord[0]))*(i-leftCoord[0]);}}}
if(typeof bezier[255]==="undefined"){bezier[255]=bezier[254];}
