...
img.src=Caman.remoteProxy+"?url="+encodeURIComponent(img.src);}}},that=this;this.options=options;if(typeof options!=="string"){if(options.image){document.addEventListener("DOMContentLoaded",function(){var canvas=document.createElement('canvas'),image=document.getElementById(options.image.substr(1));img=image;canvas.id=image.id;remoteCheck();image.parentNode.replaceChild(canvas,image);Caman.ready=true;img.onload=function(){imageReady.call(that,canvas);};},false);}else{img.src=options.src;remoteCheck();img.onload=function(){imageReady.call(that);};if(!Caman.ready){document.addEventListener("DOMContentLoaded",function(){Caman.ready=true;},false);}}}else{return Caman.store[options];}
