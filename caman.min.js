...
Caman.events.fn[_type].push(_fn);return true;}},cache:{}};(function(Caman){Caman.forEach(["trigger","listen"],function(key){Caman[key]=Caman.events.fn[key];});})(Caman);Caman.manip.pixelInfo=function(loc){var self=this,getPixelLoc=function(loc,horiz_offset,vert_offset){return loc+(self.dimensions.width*(vert_offset*-1))+(4*horiz_offset);};return{loc:loc,getPixelRelative:function(horiz_offset,vert_offset){var newLoc=getPixelLoc(this.loc,horiz_offset,vert_offset);if(newLoc>self.pixel_data.length||newLoc<0){return false;}
return{r:self.pixel_data[newLoc],g:self.pixel_data[newLoc+1],b:self.pixel_data[newLoc+2],a:self.pixel_data[newLoc+3]};},putPixelRelative:function(horiz_offset,vert_offset,rgba){var newLoc=getPixelLoc(this.loc,horiz_offset,vert_offset);if(newLoc>self.pixel_data.length||newLoc<0){return false;}
self.pixel_data[newLoc]=rgba.r;self.pixel_data[newLoc+1]=rgba.g;self.pixel_data[newLoc+2]=rgba.b;self.pixel_data[newLoc+3]=rgba.a;},getPixel:function(x,y){var newLoc=(y*self.dimensions+x)*4;return{r:self.pixel_data[newLoc],g:self.pixel_data[newLoc+1],b:self.pixel_data[newLoc+2],a:self.pixel_data[newLoc+3]};},putPixel:function(x,y,rgba){var newLoc=(y*self.dimensions+x)*4;self.pixel_data[newLoc]=rgba.r;self.pixel_data[newLoc+1]=rgba.g;self.pixel_data[newLoc+2]=rgba.b;self.pixel_data[newLoc+3]=rgba.a;}};};Caman.manip.executeFilter=function(adjust,processFn){var n=this.pixel_data.length,res=null,blockPixelLength=Math.floor((n/4)/Caman.renderBlocks),blockN=blockPixelLength*4,lastBlockN=blockN+((n/4)%Caman.renderBlocks)*4,self=this,render_block=function(bnum,start,end){console.log("BLOCK #"+bnum+" - Filter: "+processFn.name+", Start: "+start+", End: "+end);setTimeout(function(){for(var i=start;i<end;i+=4){res=processFn.call(self.pixelInfo(i),adjust,{r:self.pixel_data[i],g:self.pixel_data[i+1],b:self.pixel_data[i+2],a:self.pixel_data[i+3]});self.pixel_data[i]=res.r;self.pixel_data[i+1]=res.g;self.pixel_data[i+2]=res.b;self.pixel_data[i+3]=res.a;}
