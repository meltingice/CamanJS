 ...
 */
(function(){var forEach=Array.prototype.forEach,hasOwn=Object.prototype.hasOwnProperty,slice=Array.prototype.slice,Caman=function(){if(arguments.length==1){if(Caman.store[arguments[0]]){return Caman.store[arguments[0]];}else{return new Caman.manip.loadImage(arguments[0]);}}else if(arguments.length==2){if(Caman.store[arguments[0]]){return arguments[1].call(Caman.store[arguments[0]],Caman.store[arguments[0]]);}else{if(typeof arguments[1]==='function'){return new Caman.manip.loadImage(arguments[0],arguments[1]);}else if(typeof arguments[1]==='string'){return new Caman.manip.loadCanvas(arguments[0],arguments[1]);}}}else if(arguments.length==3){if(Caman.store[arguments[0]]){return arguments[2].call(Caman.store[arguments[1]],Caman.store[arguments[1]]);}else{return new Caman.manip.loadCanvas(arguments[0],arguments[1],arguments[2]);}}};if(!('console'in window)){window.console={log:function(){},info:function(){},error:function(){}};}
Caman.ready=false;Caman.store={};Caman.renderBlocks=4;Caman.remoteProxy="";Caman.useProxy=function(lang){var langToExt={ruby:'rb',python:'py',perl:'pl'};lang=langToExt[lang.toLowerCase()]||lang.toLowerCase();return"proxies/caman_proxy."+lang;};var remoteCheck=function(src){if(Caman.isRemote(src)){if(!Caman.remoteProxy.length){console.info("Attempting to load remote image without a configured proxy, URL: "+src);return;}else{if(Caman.isRemote(Caman.remoteProxy)){console.info("Cannot use a remote proxy for loading remote images due to same-origin policy");return;}
return Caman.remoteProxy+"?url="+encodeURIComponent(src);}}};var finishInit=function(image,canvas,callback){canvas.width=image.width;canvas.height=image.height;this.canvas=canvas;this.context=canvas.getContext("2d");this.context.drawImage(image,0,0);this.image_data=this.context.getImageData(0,0,image.width,image.height);this.pixel_data=this.image_data.data;this.dimensions={width:image.width,height:image.height};this.renderQueue=[];Caman.store[this.canvas_id]=this;callback.call(this,this);return this;};Caman.manip=Caman.prototype={loadImage:function(image_id,callback){var domLoaded,self=this,startFn=function(){var canvas=document.createElement('canvas'),image=document.getElementById(image_id.substr(1)),proxyURL=remoteCheck(image.src);canvas.id=image.id;image.parentNode.replaceChild(canvas,image);if(proxyURL){image.src=proxyURL;}
this.canvas_id=image_id;this.options={canvas:image_id,image:image.src};image.onload=function(){finishInit.call(self,image,canvas,callback);};return this;};callback=callback||function(){};domLoaded=document.getElementById(image_id.substr(1))!=null;if(domLoaded){startFn.call(this);}else{document.addEventListener("DOMContentLoaded",function(){startFn.call(self);},false);}
return this;},loadCanvas:function(url,canvas_id,callback){var domLoaded,self=this,startFn=function(){var canvas=document.getElementById(canvas_id.substr(1)),image=document.createElement('img'),proxyURL=remoteCheck(url);if(proxyURL){image.src=proxyURL;}else{image.src=url;}
this.canvas_id=canvas_id;this.options={canvas:canvas_id,image:image.src};image.onload=function(){finishInit.call(self,image,canvas,callback);};};callback=callback||function(){};domLoaded=document.getElementById(canvas_id.substr(1))!=null;if(domLoaded){startFn.call(this);}else{document.addEventListener("DOMContentLoaded",function(){startFn.call(self);},false);}
...
r=parseInt(hex.substr(0,2),16);g=parseInt(hex.substr(2,2),16);b=parseInt(hex.substr(4,2),16);return{r:r,g:g,b:b};},bezier:function(start,ctrl1,ctrl2,end,lowBound,highBound){var Ax,Bx,Cx,Ay,By,Cy,x0=start[0],y0=start[1],x1=ctrl1[0],y1=ctrl1[1],x2=ctrl2[0],y2=ctrl2[1],x3=end[0],y3=end[1],t,curveX,curveY;Cx=3*(x1-x0);Bx=3*(x2-x1)-Cx;Ax=x3-x0-Cx-Bx;Cy=3*(y1-y0);By=3*(y2-y1)-Cy;Ay=y3-y0-Cy-By;bezier={};for(var i=0;i<1000;i++){t=i/1000;curveX=Math.round((Ax*Math.pow(t,3))+(Bx*Math.pow(t,2))+(Cx*t)+x0);curveY=Math.round((Ay*Math.pow(t,3))+(By*Math.pow(t,2))+(Cy*t)+y0);if(lowBound&&curveY<lowBound){curveY=lowBound;}else if(highBound&&curveY>highBound){curveY=highBound;}
...
return bezier;},processKernel:function(adjust,kernel,divisor,bias){var val={r:0,g:0,b:0};for(var i=0;i<adjust.length;i++){for(var j=0;j<adjust[i].length;j++){val.r+=(adjust[i][j]*kernel[i][j].r);val.g+=(adjust[i][j]*kernel[i][j].g);val.b+=(adjust[i][j]*kernel[i][j].b);}}
...
Caman.trigger("processComplete",{id:self.canvas_id,completed:processFn.name});self.processNext();}},render_kernel=function(){setTimeout(function(){var kernel,pixelInfo,start,end,mod_pixel_data=new Array(n),name=adjust.name,bias=adjust.bias,divisor=adjust.divisor;adjust=adjust.adjust;console.log("Rendering kernel - Filter: "+name);if(adjust.length===3){kernel=[[],[],[]];start=self.dimensions.width*4;end=n-(self.dimensions.width*4);}else{kernel=[[],[],[],[],[]];start=self.dimensions.width*8;end=n-(self.dimensions.width*8);}
...
block_finished(-1);},0);};if(type===ProcessType.SINGLE){for(var j=0;j<Caman.renderBlocks;j++){var start=j*blockN,end=start+((j==Caman.renderBlocks-1)?lastBlockN:blockN);render_block(j,start,end);}}else{render_kernel();}};Caman.manip.process=function(adjust,processFn){this.renderQueue.push({adjust:adjust,processFn:processFn,type:ProcessType.SINGLE});return this;};Caman.manip.processKernel=function(name,adjust,divisor,bias){if(!divisor){divisor=0;for(var i=0;i<adjust.length;i++){for(var j=0;j<adjust[i].length;j++){divisor+=adjust[i][j];}}}
...
return rgba;});};Caman.manip.curves=function(chan,start,ctrl1,ctrl2,end){var bezier;if(typeof chan==='string'){if(chan=='rgb'){chan=['r','g','b'];}else{chan=[chan];}}
...
return this.curves('rgb',[0,0],ctrl1,ctrl2,[255,255]);};}(Caman));}());