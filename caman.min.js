 ...
 */
(function(){var forEach=Array.prototype.forEach,hasOwn=Object.prototype.hasOwnProperty,slice=Array.prototype.slice,Caman=function(options){if(typeof options==="string"){var temp=options;if(arguments.length===1){options=temp;}else if(arguments.length===2){options={image:temp,ready:arguments[1]||false};}else{options={src:temp,canvas:arguments[1]||"",ready:arguments[2]||false};}}
...
Caman.ready=false;Caman.store={};Caman.renderBlocks=4;Caman.remoteProxy="";Caman.useProxy=function(lang){var langToExt={ruby:'rb',python:'py',perl:'pl'};lang=langToExt[lang.toLowerCase()]||lang.toLowerCase();return"proxies/caman_proxy."+lang;};Caman.manip=Caman.prototype={load:function(options){var
img=document.createElement("img"),imageReady=function(loaded_canvas){var args=arguments.length,canvas_id,canvas=loaded_canvas||null,that=this;if(!args){if(options.canvas){canvas_id=options.canvas;}else{canvas_id=options.image;}}else{canvas_id=arguments[0];}
if(!canvas){if(!args&&canvas_id.substr(0,1)==="#"){canvas=document.getElementById(canvas_id.substr(1));if(!canvas){return;}}else{return Caman.store[canvas_id];}}
canvas.width=img.width;canvas.height=img.height;this.canvas=canvas;this.canvas_id=canvas_id;this.context=canvas.getContext("2d");this.context.drawImage(img,0,0);this.image_data=this.context.getImageData(0,0,img.width,img.height);this.pixel_data=this.image_data.data;this.dimensions={width:img.width,height:img.height};this.renderQueue=[];options.ready&&options.ready.call(this,this);Caman.store[canvas_id]=this;return this;},remoteCheck=function(){if(Caman.isRemote(img.src)){if(!Caman.remoteProxy.length){console.info("Attempting to load remote image without a configured proxy, URL: "+img.src);return;}else{if(Caman.isRemote(Caman.remoteProxy)){console.info("Cannot use a remote proxy for loading remote images due to same-origin policy");return;}
img.src=Caman.remoteProxy+"?url="+encodeURIComponent(img.src);}}},that=this,startFn;if(typeof options!=="string"){if(options.image){startFn=function(){var canvas=document.createElement('canvas'),image=document.getElementById(options.image.substr(1));img=image;canvas.id=image.id;remoteCheck();image.parentNode.replaceChild(canvas,image);Caman.ready=true;options.canvas=options.image;options.src=image.src;delete options.image;that.options=options;img.onload=function(){imageReady.call(that,canvas);};};if(!document.getElementById(options.image.substr(1))){document.addEventListener("DOMContentLoaded",startFn,false);}else{startFn();}}else{img.src=options.src;remoteCheck();img.onload=function(){imageReady.call(that);};this.options=options;if(!Caman.ready){document.addEventListener("DOMContentLoaded",function(){Caman.ready=true;},false);}}}else{return Caman.store[options];}
