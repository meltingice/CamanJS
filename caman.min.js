...
block_finished(bnum);},0);},blocks_done=0,block_finished=function(bnum){console.log("Block #"+bnum+" finished! Filter: "+processFn.name);blocks_done++;if(blocks_done==Caman.renderBlocks||bnum==-1){console.log("Filter "+processFn.name+" finished!");Caman.trigger("processComplete",{id:self.canvas_id,completed:processFn.name});self.processNext();}},render_kernel=function(){console.log("Rendering kernel - Filter: "+processFn.name);setTimeout(function(){var kernel,pixelInfo,start,end,mod_pixel_data=new Array(n),bias=adjust.bias,divisor=adjust.divisor;adjust=adjust.adjust;if(adjust.length===3){kernel=[[],[],[]];start=self.dimensions.width*4;end=n-(self.dimensions.width*4);}else{kernel=[[],[],[],[],[]];start=self.dimensions.width*8;end=n-(self.dimensions.width*8);}
...
block_finished(-1);},0);};if(type===ProcessType.SINGLE){for(var j=0;j<Caman.renderBlocks;j++){var start=j*blockN,end=start+((j==Caman.renderBlocks-1)?lastBlockN:blockN);render_block(j,start,end);}}else{render_kernel();}};Caman.manip.process=function(adjust,processFn){this.renderQueue.push({adjust:adjust,processFn:processFn,type:ProcessType.SINGLE});};Caman.manip.processKernel=function(adjust,divisor,bias){var data={adjust:adjust,divisor:divisor||1,bias:bias||0};this.renderQueue.push({adjust:data,processFn:Caman.processKernel,type:ProcessType.KERNEL});};Caman.manip.processNext=function(finishedFn){if(typeof finishedFn==="function"){this.finishedFn=finishedFn;}
