...
return Caman.remoteProxy+"?url="+encodeURIComponent(src);}}};var finishInit=function(image,canvas,callback){var self=this;this.pixelStack=[];this.layerStack=[];var old_height=image.height,old_width=image.width;if(image.style.width||image.style.height){if(image.style.width){image.width=parseInt(image.style.width,10);if(image.style.height){image.height=parseInt(image.style.height,10);}else{image.height=image.width*old_height/old_width;}}else if(image.style.height){image.height=parseInt(image.style.height,10);image.width=image.height*old_width/old_height;}}
canvas.width=image.width;canvas.height=image.height;this.canvas=canvas;this.context=canvas.getContext("2d");this.context.drawImage(image,0,0,image.width,image.height);this.image_data=this.context.getImageData(0,0,image.width,image.height);this.pixel_data=this.image_data.data;this.dimensions={width:image.width,height:image.height};this.renderQueue=[];Caman.store[this.canvas_id]=this;callback.call(this,this);return this;};Caman.manip=Caman.prototype={loadImage:function(image_id,callback){var domLoaded,self=this,image,proxyURL,startFn=function(){var canvas=document.createElement('canvas');if($(image_id)===null||$(image_id).nodeName.toLowerCase()!=='img'){throw"Given element ID isn't an image: "+image_id;}
canvas.id=image.id;image.parentNode.replaceChild(canvas,image);this.canvas_id=image_id;this.options={canvas:image_id,image:image.src};finishInit.call(self,image,canvas,callback);return this;};callback=callback||function(){};if(typeof image_id==="object"&&image_id.nodeName.toLowerCase()=="img"){var element=image_id;if(image_id.id){image_id=element.id;}else{image_id="caman-"+Caman.uniqid.get();element.id=image_id;}}
domLoaded=($(image_id)!==null);if(domLoaded){image=$(image_id);proxyURL=remoteCheck(image.src);if(proxyURL){image.src=proxyURL;image.onload=function(){startFn.call(self);};}else{startFn.call(this);}}else{document.addEventListener("DOMContentLoaded",function(){image=$(image_id);proxyURL=remoteCheck(image.src);if(proxyURL){image.src=proxyURL;}
...
if(canvas.width){image.width=canvas.width;}
if(canvas.height){image.height=canvas.height;}
