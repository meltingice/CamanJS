...
return this.callback=args[2];};Caman.prototype.setInitObject=function(obj){if(typeof obj==="object"){this.initObj=obj;}else{this.initObj=$(obj);}
if(Caman.NodeJS){return this.initType='node';}else{return this.initType=obj.nodeName.toLowerCase();}};Caman.prototype.setup=function(){switch(this.initType){case"node":return this.initNode();case"img":return this.initImage();case"canvas":return this.initCanvas();}};Caman.prototype.initNode=function(){var img;img=new Image();img.onload=this.loadFinished;img.onerror=function(err){throw err;};return img.src=this.initObj;};Caman.prototype.initImage=function(){this.canvas=document.createElement('canvas');return Util.copyAttributes(this.initObj,this.canvas,{except:['src']});};return Caman;})();CamanInstance=(function(){CamanInstance.Type={Image:1,Canvas:2,Unknown:3,Node:4};CamanInstance.toString=Caman.toString;function CamanInstance(args,type){var listener,_this=this;if(type==null){type=CamanInstance.Type.Canvas;}
...
Event.trigger(this,"renderStart");return this.processNext(function(){_this.context.putImageData(_this.imageData,0,0);return callback.call(_this);});};Filter.prototype.revert=function(ready){var i,pixel,_i,_len,_ref;if(ready==null){ready=function(){};}
...
Plugin.plugins={};Plugin.register=function(name,plugin){return this.plugins[name]=plugin;};Plugin.execute=function(context,name,args){return this.plugins[name].apply(context,args);};return Plugin;})();Caman.Plugin=Plugin;RenderJob=(function(){RenderJob.Blocks=Caman.NodeJS?require('os').cpus().length:4;RenderJob.execute=function(instance,job,callback){var layer,rj;rj=new RenderJob(instance,job,callback);switch(job.type){case Filter.Type.LayerDequeue:layer=instance.canvasQueue.shift();instance.executeLayer(layer);break;case Filter.Type.LayerFinished:instance.applyCurrentLayer();instance.popContext();callback();break;case Filter.Type.LoadOverlay:rj.loadOverlay(job.layer,job.src);break;case Filter.Type.Plugin:rj.executePlugin();break;default:rj.executeFilter();}
return instance;};function RenderJob(c,job,renderDone){this.c=c;this.job=job;this.renderDone=renderDone;}
RenderJob.prototype.executeFilter=function(){var blockN,blockPixelLength,end,f,j,lastBlockN,n,start,_i,_ref,_results,_this=this;this.blocksDone=0;n=this.c.pixelData.length;blockPixelLength=Math.floor((n/4)/RenderJob.Blocks);blockN=blockPixelLength*4;lastBlockN=blockN+((n/4)%RenderJob.Blocks)*4;Event.trigger(this.c,"processStart",this.job);if(this.job.type===Filter.Type.Single){_results=[];for(j=_i=0,_ref=RenderJob.Blocks;0<=_ref?_i<_ref:_i>_ref;j=0<=_ref?++_i:--_i){start=j*blockN;end=start+(j===RenderJob.Blocks-1?lastBlockN:blockN);if(Caman.NodeJS){f=Fiber(function(){return _this.renderBlock(j,start,end);});_results.push(f.run());}else{_results.push(setTimeout((function(j,start,end){return function(){return _this.renderBlock(j,start,end);};})(j,start,end),0));}}
