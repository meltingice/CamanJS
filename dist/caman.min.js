...
Root.Caman=Caman=(function(){Caman.version={release:"3.4.0",date:"12/17/12"};Caman.DEBUG=false;Caman.NodeJS=typeof exports!=="undefined"&&exports!==null;Caman.autoload=!Caman.NodeJS;Caman.crossOrigin="anonymous";Caman.toString=function(){return"Version "+Caman.version.release+", Released "+Caman.version.date;};Caman.remoteProxy="";function Caman(){this.finishInit=__bind(this.finishInit,this);var args,_this=this;if(arguments.length===0){throw"Invalid arguments";}
if(this instanceof Caman){args=arguments[0];this.id=Util.uniqid.get();this.originalPixelData=[];this.pixelStack=[];this.layerStack=[];this.canvasQueue=[];this.currentLayer=null;this.scaled=false;this.analyze=new Analyze(this);this.renderer=new Renderer(this);this.domIsLoaded(function(){_this.parseArguments(args);return _this.setup();});return this;}else{return new Caman(arguments);}}
Caman.prototype.domIsLoaded=function(cb){var listener,_this=this;if(Caman.NodeJS){return cb.call(this);}
if(document.readyState==="complete"){return cb.call(this);}else{listener=function(){if(document.readyState==="complete"){return cb.call(_this);}};return document.addEventListener("readystatechange",listener,false);}};Caman.prototype.parseArguments=function(args){if(args.length===0){throw"Invalid arguments given";}
...
return this.initType=this.initObj.nodeName.toLowerCase();};Caman.prototype.setup=function(){switch(this.initType){case"node":return this.initNode();case"img":return this.initImage();case"canvas":return this.initCanvas();}};Caman.prototype.initNode=function(){var _this=this;this.image=new Image();this.image.onload=function(){_this.canvas=new Canvas(_this.image.width,_this.image.height);return _this.finishInit();};this.image.onerror=function(err){throw err;};return this.image.src=this.initObj;};Caman.prototype.initImage=function(){var _this=this;this.image=this.initObj;this.canvas=document.createElement('canvas');Util.copyAttributes(this.image,this.canvas,{except:['src']});return this.imageLoaded(function(){_this.canvas.width=_this.image.width;_this.canvas.height=_this.image.height;if(_this.needsHiDPISwap()){Log.debug(_this.image.src,"->",_this.hiDPIReplacement());_this.image.src=_this.hiDPIReplacement();return _this.imageLoaded(function(){Log.debug("HiDPI version loaded");_this.swapped=true;_this.image.parentNode.replaceChild(_this.canvas,_this.image);return _this.finishInit();});}else{_this.image.parentNode.replaceChild(_this.canvas,_this.image);return _this.finishInit();}});};Caman.prototype.initCanvas=function(){if(this.imageUrl!=null){this.image=document.createElement('img');this.image.src=this.imageUrl;return this.imageLoaded(this.finishInit);}else{return this.finishInit();}};Caman.prototype.imageLoaded=function(cb){if(this.image.complete){return cb();}else{return this.image.onload=cb;}};Caman.prototype.finishInit=function(){var pixel,_i,_len,_ref;this.assignId();this.context=this.canvas.getContext('2d');this.originalWidth=this.width=this.canvas.width;this.originalHeight=this.height=this.canvas.height;this.hiDPIAdjustments();if(this.image!=null){this.context.drawImage(this.image,0,0,this.image.width,this.image.height,0,0,this.originalWidth,this.originalHeight);}
