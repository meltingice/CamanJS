...
return this.initType=this.initObj.nodeName.toLowerCase();};Caman.prototype.setup=function(){switch(this.initType){case"node":return this.initNode();case"img":return this.initImage();case"canvas":return this.initCanvas();}};Caman.prototype.initNode=function(){var _this=this;this.image=new Image();this.image.onload=function(){_this.canvas=new Canvas(_this.image.width,_this.image.height);return _this.finishInit();};this.image.onerror=function(err){throw err;};return this.image.src=this.initObj;};Caman.prototype.initImage=function(){var _this=this;this.image=this.initObj;this.canvas=document.createElement('canvas');Util.copyAttributes(this.image,this.canvas,{except:['src']});this.image.parentNode.replaceChild(this.canvas,this.image);return this.imageLoaded(function(){_this.canvas.width=_this.image.width;_this.canvas.height=_this.image.height;if(_this.needsHiDPISwap()){Log.debug(_this.image.src,"->",_this.hiDPIReplacement());_this.image.src=_this.hiDPIReplacement();return _this.imageLoaded(function(){Log.debug("HiDPI version loaded");_this.swapped=true;return _this.finishInit();});}else{return _this.finishInit();}});};Caman.prototype.initCanvas=function(){if(this.imageUrl!=null){this.image=document.createElement('img');this.image.src=this.imageUrl;return this.imageLoaded(this.finishInit);}else{return this.finishInit();}};Caman.prototype.imageLoaded=function(cb){if(this.image.complete){return cb();}else{return this.image.onload=cb;}};Caman.prototype.finishInit=function(){var pixel,_i,_len,_ref;this.context=this.canvas.getContext('2d');this.originalWidth=this.width=this.canvas.width;this.originalHeight=this.height=this.canvas.height;this.hiDPIAdjustments();this.assignId();if(this.image!=null){this.context.drawImage(this.image,0,0,this.image.width,this.image.height,0,0,this.originalWidth,this.originalHeight);}
