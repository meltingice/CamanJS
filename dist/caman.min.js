...
this.canvasID=id;this.options={canvas:id,image:this.image.src,crossOrigin:this.image.crossOrigin};return this.finishInit(callback);};CamanInstance.prototype.loadCanvas=function(url,xorigin,id,callback){var element,_ref;if(typeof id==="object"&&((_ref=id.nodeName)!=null?_ref.toLowerCase():void 0)==="canvas"){element=id;if(!element.id){element.id="caman-"+(Util.uniqid.get());}}else{if($(id)!=null){element=$(id);}else{throw"Could not find element "+id;}}
return this.canvasLoaded(url,xorigin,element,callback);};CamanInstance.prototype.canvasLoaded=function(url,xorigin,canvas,callback){var proxyURL,_this=this;this.canvas=canvas;if(!canvas||canvas.nodeName.toLowerCase()!=="canvas"){throw"Given element ID isn't a canvas: "+id;}
if(url!=null){this.image=document.createElement('img');this.image.onload=function(){return _this.finishInit(callback);};proxyURL=IO.remoteCheck(url);this.canvasID=this.canvas.id;this.options={canvas:canvas.id,image:url,crossOrigin:xorigin};if(xorigin!==""){this.image.crossOrigin=""+xorigin;}
return this.image.src=proxyURL?proxyURL:url;}else{return this.finishInit(callback);}};CamanInstance.prototype.loadNode=function(file,callback){var img,_this=this;img=new Image();if(typeof file==="string"){file=fs.realpathSync(file);}
img.onload=function(){var context;_this.canvasID=Util.uniqid.get();_this.canvas=new Canvas(img.width,img.height);context=_this.canvas.getContext('2d');context.drawImage(img,0,0);return _this.finishInit(callback);};img.onerror=function(err){throw err;};return img.src=file;};CamanInstance.prototype.finishInit=function(callback){var newHeight,newWidth,oldHeight,oldWidth;if(callback==null){callback=function(){};}
